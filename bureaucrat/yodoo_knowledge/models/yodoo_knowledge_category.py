...
class YodooKnowledgeCategory(models.Model):
    _name = 'yodoo.knowledge.category'
    _description = "Yodoo Knowledge: Category"
    _parent_store = True
    ...
    parent_ids = fields.Many2manyView(
        comodel_name='yodoo.knowledge.category',
        relation='yodoo_knowledge_category_parents_rel_view',
        column1='child_id',
        column2='parent_id',
    ...
    visibility_group_ids = fields.Many2many(
        ...
        relation='yodoo_knowledge_category_visibility_groups',
        column1='knowledge_category_id',
        column2='group_id',
        string='Readers groups')
    visibility_user_ids = fields.Many2many(
        comodel_name='res.users',
        relation='yodoo_knowledge_category_visibility_users',
        column1='knowledge_category_id',
        column2='user_id',
        string='Readers')
    ...
    editor_group_ids = fields.Many2many(
        comodel_name='res.groups',
        relation='yodoo_knowledge_category_editor_groups',
        column1='knowledge_category_id',
        column2='group_id',
        string='Editors groups')
    actual_editor_group_ids = fields.Many2manyView(
        comodel_name='res.groups',
        relation='yodoo_knowledge_category_actual_editor_groups_rel_view',
        column1='knowledge_category_id',
        column2='group_id',
    ...
    editor_user_ids = fields.Many2many(
        comodel_name='res.users',
        relation='yodoo_knowledge_category_editor_users',
        column1='knowledge_category_id',
        column2='user_id',
        string='Editors')
    actual_editor_user_ids = fields.Many2manyView(
        comodel_name='res.users',
        relation='yodoo_knowledge_category_actual_editor_users_rel_view',
        column1='knowledge_category_id',
        column2='user_id',
    ...
    owner_group_ids = fields.Many2many(
        comodel_name='res.groups',
        relation='yodoo_knowledge_category_owner_groups',
        column1='knowledge_category_id',
        column2='group_id',
        string='Owners groups')
    actual_owner_group_ids = fields.Many2manyView(
        comodel_name='res.groups',
        relation='yodoo_knowledge_category_actual_owner_groups_rev_view',
        column1='knowledge_category_id',
        column2='group_id',
    ...
    owner_user_ids = fields.Many2many(
        comodel_name='res.users',
        relation='yodoo_knowledge_category_owner_users',
        column1='knowledge_category_id',
        column2='user_id',
        string='Owners')
    actual_owner_user_ids = fields.Many2manyView(
        comodel_name='res.users',
        relation='yodoo_knowledge_category_actual_owner_users_rel_view',
        column1='knowledge_category_id',
        column2='user_id',
    ...
    def _compute_items_count(self):
        for rec in self:
            rec.items_count = len(rec.item_ids)

    @api.depends('child_ids', 'item_ids')
    def _compute_category_contents(self):
        tmpl = self.sudo().env.ref(
            'yodoo_knowledge.knowledge_category_content_template')
        for rec in self:
            if rec.child_ids or rec.item_ids:
                val = {
                    'user': self.env.user,
                    'env': self.env,
                    'category': rec,
    ...
    def init(self):
        ...
        tools.drop_view_if_exists(
            self.env.cr, 'yodoo_knowledge_category_parents_rel_view')
        self.env.cr.execute(sql.SQL("""
            CREATE or REPLACE VIEW
                yodoo_knowledge_category_parents_rel_view AS (
                SELECT bkc.id          AS child_id,
                       bkc_parent.id   AS parent_id
                FROM yodoo_knowledge_category AS bkc
                LEFT JOIN yodoo_knowledge_category AS bkc_parent ON (
        ...
        tools.drop_view_if_exists(
            self.env.cr,
            'yodoo_knowledge_category_actual_owner_groups_rev_view')
        self.env.cr.execute(sql.SQL("""
           CREATE or REPLACE VIEW
              yodoo_knowledge_category_actual_owner_groups_rev_view AS (
               SELECT DISTINCT
                    parent_ids.child_id AS knowledge_category_id,
                    own_group.group_id     AS group_id
               FROM yodoo_knowledge_category_parents_rel_view
                    AS parent_ids
               JOIN yodoo_knowledge_category_owner_groups AS own_group
               ON (
        ...
        tools.drop_view_if_exists(
            self.env.cr,
            'yodoo_knowledge_category_actual_owner_users_rel_view')
        self.env.cr.execute(sql.SQL("""
           CREATE or REPLACE VIEW
               yodoo_knowledge_category_actual_owner_users_rel_view AS (
               SELECT DISTINCT
                    parent_ids.child_id AS knowledge_category_id,
                    own_usr.user_id     AS user_id
               FROM yodoo_knowledge_category_parents_rel_view
                    AS parent_ids
               JOIN yodoo_knowledge_category_owner_users AS own_usr ON (
        ...
        tools.drop_view_if_exists(
            self.env.cr,
            'yodoo_knowledge_category_actual_editor_groups_rel_view')
        self.env.cr.execute(sql.SQL("""
           CREATE or REPLACE VIEW
              yodoo_knowledge_category_actual_editor_groups_rel_view AS (
               SELECT DISTINCT
                    parent_ids.child_id AS knowledge_category_id,
                    editor_group.group_id     AS group_id
               FROM yodoo_knowledge_category_parents_rel_view
                    AS parent_ids
               JOIN yodoo_knowledge_category_editor_groups AS editor_group
               ON (
        ...
        tools.drop_view_if_exists(
            self.env.cr,
            'yodoo_knowledge_category_actual_editor_users_rel_view')
        self.env.cr.execute(sql.SQL("""
           CREATE or REPLACE VIEW
               yodoo_knowledge_category_actual_editor_users_rel_view AS (
               SELECT DISTINCT
                    parent_ids.child_id AS knowledge_category_id,
                    editor_usr.user_id     AS user_id
               FROM yodoo_knowledge_category_parents_rel_view
                    AS parent_ids
               JOIN yodoo_knowledge_category_editor_users AS editor_usr
               ON (
    ...
    def create(self, vals):
        ...
        # access rights later
        categories = super(
            YodooKnowledgeCategory, self.sudo()).create(values)
    ...
    def write(self, vals):
        res = super(YodooKnowledgeCategory, self).write(vals)
    ...
    def action_view_subcategories(self):
        ...
        return self.env['generic.mixin.get.action'].get_action_by_xmlid(
            'yodoo_knowledge.action_yodoo_knowledge_category',
    ...
    def action_view_items(self):
        self.ensure_one()
        return self.env['generic.mixin.get.action'].get_action_by_xmlid(
            'yodoo_knowledge.action_yodoo_knowledge_item',
    ...
    def _post_active_changed(self, changes):
        for rec in self:
            ...
            self.env['yodoo.knowledge.item'].with_context(
