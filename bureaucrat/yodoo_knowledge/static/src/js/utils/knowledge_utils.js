...
export function getPropNameNode(propName, anchor) {
    ...
    for (let i = propNodes.length - 1; i >= 0; i--) {
        const closest = propNodes[i].closest('.o_yodoo_knowledge_behavior_anchor');
        if (closest === anchor) {
            return propNodes[i];
        }
    }
...
export function getPropNameNodes(anchor) {
    ...
    const subAnchors = [...anchor.querySelectorAll(".o_yodoo_knowledge_behavior_anchor")];
 ...
 *
 * Warning: Can only be used on a Component which implements YodooAbstractBehavior.
 *
