...
class GenericRequestFieldSetMixin(models.AbstractModel):
    _name = 'generic.request.field.set.mixin'
    _inherit = 'generic.mixin.track.changes'
    _description = 'Generic Request Field Set (Mixin)'
    ...
    def _post_create_request_id_trigger_event(self, changes):
        if self.request_id:
            self.request_id.trigger_event('fieldset-created')
