...
import { KnowledgeItemBehavior } from "@yodoo_knowledge/js/components/behaviors/knowledge_item_behavior/knowledge_item_behavior";
...
const HtmlFieldPatch = {
    ...
    setup() {
        ...
        this.yodooBehaviorTypes = {
            o_yodoo_knowledge_behavior_type_knowledge_item: {
                Behavior: KnowledgeItemBehavior,
            },
        };
        this.uiService = useService('ui');
        this.yodooBehaviorState = {
            ...
            appAnchorsObserver: new MutationObserver(() => {
                ...
                const anchors = this.yodooBehaviorState.observedElement.querySelectorAll('.o_yodoo_knowledge_behavior_anchor');
                ...
                const hiddenMountedAnchors = [
                    ...(this.yodooBehaviorState.handlerRef.el?.querySelectorAll(
                        ".o_yodoo_knowledge_behavior_anchor"
                    ) || []),
                ].filter((anchor) => {
            ...
            handlerRef: useRef("yodooBehaviorHandler"),
        ...
        this.yodooKnowledgeCommandsService = useService('yodooKnowledgeCommandsService');
        onWillDestroy(() => {
            this.yodooBehaviorState.validator.obsolete = true;
            this.yodooBehaviorState.appAnchorsObserver.disconnect();
            this.destroyBehaviorApps();
        });
        onWillUnmount(() => {
            this._removeMountBehaviorsListeners();
        });
        let previousKnowledgeItemId;
        useRecordObserver((record, props) => {
            if (!previousKnowledgeItemId || previousKnowledgeItemId !== record.resId) {
                previousKnowledgeItemId = record.resId;
        ...
        useEffect(() => {
            ...
            // DOM element changes.
            if (this.yodooBehaviorState.observedElement !== this.valueContainerElement) {
                // The observed Element has to be replaced.
                this.yodooBehaviorState.appAnchorsObserver.disconnect();

                this.yodooBehaviorState.observedElement = null;
                this.destroyBehaviorApps();
            }
            if (
                !this.yodooBehaviorState.observedElement &&
    ...
    get wysiwygOptions() {
        ...
        const mountCollaborativeBehaviors = (element) => {
            ...
            if (!this.yodooBehaviorState.updateMutex._unlockedProm) {
    ...
    async startWysiwyg() {
        ...
        const behaviorBlueprint = this.yodooKnowledgeCommandsService.popPendingBehaviorBlueprint({
    ...
    async _toInline() {
        ...
        await this.yodooBehaviorState.updateMutex.getUnlockedDef();
    ...
    destroyBehaviorApp(anchor) {
        ...
        if (this.yodooBehaviorState.handlerRef.el?.contains(anchor)) {
            ...
            if (this.yodooBehaviorState.handlerRef.el) {
                // A Component should always be destroyed in the DOM.
                this.yodooBehaviorState.handlerRef.el.append(anchor);
            } else {
        ...
        this.yodooBehaviorState.appAnchors.delete(anchor);
        this.yodooBehaviorState.obsoleteAnchors.delete(anchor);
    },
    ...
    destroyBehaviorApps(ignoredAnchors=new Set()) {
        for (const anchor of Array.from(this.yodooBehaviorState.appAnchors)) {
            if (this.yodooBehaviorState.obsoleteAnchors.has(anchor) || !ignoredAnchors.has(anchor)) {
    ...
    updateBehaviorsValidator(props) {
        ...
        if (
            Object.entries(this.yodooBehaviorState.validator.state).some(
            ...
            this.yodooBehaviorState.validator = validator;
        }
    },
     ...
     *                      Behavior @see yodooBehaviorTypes
    ...
    async mountBehaviors(behaviorsData = [], target = null) {
        for (const behaviorData of behaviorsData) {
            behaviorData.validator = this.yodooBehaviorState.validator;
        }
        this.yodooBehaviorState.updateMutex.exec(() => this._mountBehaviors(behaviorsData, target));
        return this.yodooBehaviorState.updateMutex.getUnlockedDef();
    },
    ...
    async _mountBehaviors(behaviorsData, target) {
        if (
            ...
            !document.body.contains(this.yodooBehaviorState.handlerRef.el)
        ) {
            // Validate that the working environment is ready.
            return;
        }
        const renderingContainerElement = (this.props.readonly) ? target || this.readonlyElementRef.el : this.yodooBehaviorState.handlerRef.el;
        ...
        const promises = [];
        for (const behaviorData of behaviorsData) {
            const {Behavior} = this.yodooBehaviorTypes[behaviorData.behaviorType] || {};
            if (
                !Behavior ||
            ...
            this.yodooBehaviorState.appAnchors.add(anchor);
            anchor.oKnowledgeBehavior.validator = this.yodooBehaviorState.validator;
        ...
        if (!this.props.readonly) {
            ...
            props.onReadyToInsertInEditor = () => {
                const cancelInsertion = () => {
                    this.yodooBehaviorState.obsoleteAnchors.add(anchor);
                };
                if (
                    behaviorData.validator.obsolete ||
    ...
    _scanFieldForBehaviors(target) {
        const behaviorsData = [];
        const types = new Set(Object.getOwnPropertyNames(this.yodooBehaviorTypes));
        const anchors = target.querySelectorAll('.o_yodoo_knowledge_behavior_anchor');
        ...
        for (const anchor of anchors) {
            ...
            if (anchor.oKnowledgeBehavior) {
                ...
                const anchorSubNodes = anchor.querySelectorAll('.o_yodoo_knowledge_behavior_anchor');
        ...
        for (const anchor of anchorsSet) {
            ...
            if (type) {
                behaviorsData.push({
                    anchor: anchor,
                    behaviorType: type,
                    validator: this.yodooBehaviorState.validator,
                });
            }
        }
    ...
    async _lazyloadWysiwyg() {
        ...
        let wysiwygModule = await odoo.loader.modules.get('@yodoo_knowledge/js/wysiwyg');
        if (!wysiwygModule) {
            await loadBundle('yodoo_knowledge.assets_wysiwyg');
        }
    },
    ...
    startAppAnchorsObserver() {
        this.yodooBehaviorState.observedElement = this.valueContainerElement;
        this.yodooBehaviorState.appAnchorsObserver.observe(this.yodooBehaviorState.observedElement, {
            subtree: true,
            childList: true,
        });
    },
